...
#define ADC_TIMER_PERIOD                                                       \
  125 // uS (determines the sampling rate / amount of idle time)
...
//
const float lpf_gain = 6; // <- setting this to 0 disables this extra processing
...
void setup() {
  ...
  DCoffset_V_min =
      (long)(512L - 100) * 256; // mid-point of ADC minus a working margin
  DCoffset_V_max =
      (long)(512L + 100) * 256; // mid-point of ADC plus a working margin

  // Set up the ADC to be triggered by a hardware timer of fixed duration
  ADCSRA = (1 << ADPS0) + (1 << ADPS1) +
           (1 << ADPS2); // Set the ADC's clock to system clock / 128
  ADCSRA |= (1 << ADEN); // Enable ADC

  Timer1.initialize(ADC_TIMER_PERIOD); // set Timer1 interval
  Timer1.attachInterrupt(
      timerIsr); // declare timerIsr() as interrupt service routine
...
void timerIsr(void) {
  ...
  case 0:
    ...
    ADMUX =
        0x40 +
        sensor_I1; // set up the next conversion, which is for current at CT1
  ...
  case 1:
    ...
    ADMUX =
        0x40 +
        sensor_I2; // set up the next conversion, which is for current at CT2
    ADCSRA |= (1 << ADSC); // start the ADC
    sample_index++;        // increment the control flag
    break;
  case 2:
...
void allGeneralProcessing() // each iteration is for one set of data samples
{
  ...
  if (firstLoop) {
    ...
    noOfCyclesToBeRecorded =
        3; // more array space may be needed if this value is >1 !!!
  ...
  if (polarityOfMostRecentVsample == POSITIVE) {
    ...
    if ((sampleSetsDuringThisHalfMainsCycle == 3) &&
        (cycleNumberBeingRecorded == 1)) {
  ...
  else // the polatity of this sample is negative
  {
    ...
    if ((sampleSetsDuringThisHalfMainsCycle == 3) &&
        (cycleNumberBeingRecorded == 1)) {
      digitalWrite(outputForTrigger,
                   LOAD_OFF); // triac will release at the next ZC point
    }
...
void dispatch_recorded_data() {
  ...
  for (int index = 0; index < samplesRecorded; index++) {
    ...
    if ((I1 > lowerLimit) && (I1 < upperLimit)) {
      newLine[map(I1, lowerLimit, upperLimit, 0, 80)] =
          '1'; // <-- raw sample scale
    }
